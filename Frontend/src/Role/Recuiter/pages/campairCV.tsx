...
export default function CVComparison() {
  ...
  useEffect(() => {
    ...
    axios.get(`'http://localhost:3000/jobApplicationControl/getApplications`)
      .then(res => {
        const formattedCandidates = res.data.data.map((candidate: any) => ({
          id: candidate._id,
          name: candidate.userId?.yourName || candidate.name || "",
          specialty: candidate.userId?.specialization || "",
          graduationYear: candidate.userId?.graduationDate
            ? new Date(candidate.userId.graduationDate).getFullYear()
            : "N/A",
          university: candidate.userId?.university || "",
          experience: candidate.userId?.experience || "",
          medicalDegree: candidate.userId?.medicalDegree || "",
          location: candidate.userId?.currentLocation || "",
          certifications: candidate.userId?.certificationInput || [],
          skills: (candidate.userId?.careerSummary || "")
            .split(",")
            .map((t: string) => t.trim())
            .filter(Boolean),
          cvFile: {
            name: candidate.userId?.resumeRawUrl?.split("/").pop() || "CV.pdf",
            uploadDate: candidate.updatedAt
              ? new Date(candidate.updatedAt).toLocaleDateString()
              : "",
            size: "-",
            url: candidate.userId?.resumeRawUrl,
          },
          resumeRawUrl: candidate.userId?.resumeRawUrl,
          resumePdfUrl: candidate.userId?.resumeRawUrl,
          resumeDownloadUrl: candidate.userId?.resumeRawUrl,
          linkedinUrl: candidate.userId?.linkedinLink,
        }));
        setCandidatesSource(formattedCandidates);
  
        // Fetch specialties
        axios
          .get("http://localhost:3000/api/recruiter/candidates/specialties")
          .then(specsRes => {
        ...
        axios
          .get("http://localhost:3000/api/recruiter/candidates/years")
          .then(yearsRes => {
